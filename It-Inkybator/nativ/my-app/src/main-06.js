...
const sortedByName2 = students.sort((a, b)=> a.name.localeCompare(b.name));
...
console.log(bestStudents.sort((a,b)=>b.scores===a.scores ? 0 : b.scores > a.scores ? 1:-1 ).slice(0,3))
...
console.log(students.filter(t=>t.isMarried===false))
...
console.log(students.map(t=>t.name))
// const studentsNames = students.map((st) => st.name);
// console.log(studentsNames);
// //8a. Сформируйте строку из имён студентов, разделённых
console.log(students.map(t=>t.name).join(" "))
console.log(students.map(t=>t.name).join(","))
...
console.log(students.map(t=>t.name==="Nick"? {...t, isMarried:true} : t));
...
console.log(students.find(t=>t.name==="Ann"));
...
console.log(students.sort((a,b)=>b.scores-a.scores)[0])
console.log(students.sort((a,b)=>b.scores-a.scores)[1])
...
//   console.log(addFriends(students));
  ...
  const addNewStudents = (std) => {

  return std.map(obj=>({...obj,friends:std.map(n=>n.name).filter(t=>t!==obj.name)}))
  }
console.log(addNewStudents(students))
